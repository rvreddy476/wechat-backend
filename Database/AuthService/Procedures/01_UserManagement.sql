...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM auth.Users WHERE Email = p_Email AND IsDeleted = FALSE) THEN
        RETURN QUERY SELECT NULL::UUID, FALSE, 'Email already registered'::TEXT;
        RETURN;
    END IF;

    -- Check if username already exists
    IF EXISTS (SELECT 1 FROM auth.Users WHERE Username = p_Username AND IsDeleted = FALSE) THEN
        RETURN QUERY SELECT NULL::UUID, FALSE, 'Username already taken'::TEXT;
        RETURN;
    END IF;

    -- Insert new user
    INSERT INTO auth.Users (Username, Email, PasswordHash, PhoneNumber)
    VALUES (p_Username, p_Email, p_PasswordHash, p_PhoneNumber)
    ...
    RETURN QUERY SELECT v_UserId, TRUE, 'User registered successfully'::TEXT;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    GROUP BY u.UserId, u.Username, u.Email, u.EmailVerified, u.PhoneNumber,
             u.PhoneNumberVerified, u.TwoFactorEnabled, u.IsActive, u.CreatedAt, u.LastLoginAt;
END;
$$ LANGUAGE plpgsql;
...
COMMENT ON FUNCTION auth.sp_RegisterUser IS 'Registers a new user with default User role';
COMMENT ON FUNCTION auth.sp_AuthenticateUser IS 'Authenticates user and returns user data with roles';
COMMENT ON FUNCTION auth.sp_GetUserById IS 'Gets user details by UserId';
COMMENT ON FUNCTION auth.sp_UpdateUserProfile IS 'Updates user profile information';
